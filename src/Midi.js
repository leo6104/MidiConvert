...
class Midi {
  ...
  decode(bytes){
    ...
    midiData.tracks.forEach((trackData, i) => {
      ...
      let absoluteTime = 0;
      trackData.forEach((event) => {
        if (event.subtype === 'controller' || event.subtype === 'noteOn' || event.subtype === 'noteOff') {
          absoluteTime += Util.ticksToSeconds(event.deltaTime, this.header, bpm)
          bpm = this.header.bpm;
          for (let i = 0; i < track.tempos; i++) {
            if (absoluteTime > track.tempos[i].deltaTime) {
              bpm = track.tempos[i].bpm;
            }
          }
        }
